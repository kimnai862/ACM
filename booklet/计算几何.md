...
半径固定的圆最多能覆盖多少个点$n^2log(n)$ 

```
struct Angle{
    double ang;
    int in;
    Angle(double _ang=0,int _in=0){ang=_ang;in=_in;}
};
Angle v[605];
int tmp(Angle a,Angle b){
...
int work(double r){
    ...
    for(int i=1;i<=n;i++){
        ...
        for(int j=1;j<=n;j++){
            ...
            double phi=acos(dist(p[i],p[j])/(2*r));
            v[tot++]=Angle(angle-phi,1);
            v[tot++]=Angle(angle+phi,0);
        }
        if(tot<s)continue;
        sort(v,v+tot,tmp);
        int cnt=1;
